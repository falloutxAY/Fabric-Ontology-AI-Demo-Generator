...
1. **Use `GROUP BY` with grouped aggregations**: When using `count()`, `sum()`, `avg()`, etc. with grouping dimensions, include a `GROUP BY` clause.
2. **Use either `WHERE` or `FILTER`**: Official Fabric docs allow both `MATCH ... WHERE ...` and `FILTER [WHERE] ...`.
3. **Use `ZONED_DATETIME()`**: For zoned datetime literals, use `ZONED_DATETIME('2025-01-10T00:00:00Z')`.
4. **Use `LET` for derived variables when helpful**: `LET` is useful for readability and reusable aliases.
...
```gql
-- Preferred pattern with LET aliases
MATCH (n:Entity)
