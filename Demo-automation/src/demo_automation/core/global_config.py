...
class GlobalConfig:
    ...
    environment: str = "production"  # production or custom
    api_base: Optional[str] = None   # Custom API base URL (only used if environment=custom)
    api_scope: Optional[str] = None  # Custom API scope (only used if environment=custom)
    ...
    # All environments share: same paths, same auth model, same payloads.
    # Only the API host and OneLake host change between environments.
    # Auth scope is always https://api.fabric.microsoft.com/.default
    ENVIRONMENT_URLS = {
        "production": {
            "api_base": "https://api.fabric.microsoft.com",
            "onelake_url": "https://onelake.dfs.fabric.microsoft.com",
        },
    }
    
    # Auth scope is the same for all environments
    API_SCOPE = "https://api.fabric.microsoft.com/.default"
    
    def get_api_base(self) -> str:
        """Get the API base URL for the configured environment."""
        if self.environment == "custom" and self.api_base:
            return self.api_base
        return self.ENVIRONMENT_URLS.get(self.environment, self.ENVIRONMENT_URLS["production"])["api_base"]
    
    def get_api_scope(self) -> str:
        """Get the API scope (same for all environments)."""
        if self.environment == "custom" and self.api_scope:
            return self.api_scope
        return self.API_SCOPE
    
    def get_onelake_url(self) -> str:
        """Get the OneLake URL for the configured environment."""
        if self.environment == "custom":
            # For custom, default to production OneLake unless overridden via env var
            return os.environ.get('ONELAKE_URL', self.ENVIRONMENT_URLS["production"]["onelake_url"])
        return self.ENVIRONMENT_URLS.get(self.environment, self.ENVIRONMENT_URLS["production"])["onelake_url"]
    
    def apply_environment_variables(self) -> None:
        """Set environment variables based on configuration (call before SDK/client init)."""
        api_base = self.get_api_base()
        api_scope = self.get_api_scope()
        onelake_url = self.get_onelake_url()
        
        # Only set if not already set by user
        if not os.environ.get('FABRIC_API_BASE'):
            os.environ['FABRIC_API_BASE'] = api_base
        if not os.environ.get('FABRIC_API_SCOPE'):
            os.environ['FABRIC_API_SCOPE'] = api_scope
        if not os.environ.get('ONELAKE_URL'):
            os.environ['ONELAKE_URL'] = onelake_url
    ...
    def _from_dict(cls, data: Dict[str, Any]) -> "GlobalConfig":
        ...
        return cls(
            ...
            environment=environment_config.get('name', 'production'),
            api_base=environment_config.get('api_base'),
            api_scope=environment_config.get('api_scope'),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        # Only include custom URLs if environment is custom
        if self.environment == "custom":
            result['environment']['api_base'] = self.api_base
            result['environment']['api_scope'] = self.api_scope
        return result
