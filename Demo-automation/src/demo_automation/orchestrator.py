...
from .platform.lakehouse_client import ParallelLoadConfig, RetryConfig
...
class DemoOrchestrator:
    ...
    def _handle_resume(self, existing_state) -> None:
        ...
        # Restore bindings_configured flag from completed steps
        completed = existing_state.get_completed_steps()
        if "configure_bindings" in completed or (
            "bind_static" in completed
            and "bind_timeseries" in completed
            and "bind_relationships" in completed
        ):
    ...
    def _get_existing_lakehouse_tables(self) -> set:
        ...
        Uses the Fabric Tables API first. If that fails (e.g., PrivateLinkValidationFailure),
        falls back to listing the Tables/ directory via the OneLake ADLS Gen2 API.
        """
        ...
        try:
            ...
            logger.warning(f"Could not list lakehouse tables via Fabric API: {e}")
            # Fallback: list Tables/ directory via OneLake ADLS Gen2
            try:
                logger.info("Falling back to OneLake ADLS to list lakehouse tables")
                item_path = self.onelake_client._get_item_path(
                    self.state.lakehouse_id,
                    item_name=self.state.lakehouse_name,
                    item_type="Lakehouse",
                )
                tables_path = f"{item_path}/Tables"
                dir_client = self.onelake_client.file_system_client.get_directory_client(tables_path)
                table_names = set()
                for path in dir_client.get_paths():
                    if path.is_directory:
                        name = path.name.split("/")[-1]
                        # Skip hidden/system directories (e.g., _delta_log)
                        if not name.startswith("_"):
                            table_names.add(name)
                logger.info(f"OneLake fallback found {len(table_names)} tables")
                return table_names
            except Exception as fallback_err:
                logger.warning(f"OneLake fallback also failed: {fallback_err}")
                return set()
    ...
    def _step_upload_lakehouse_files(self) -> StepResult:
        ...
        # Filter to only upload files for tables that don't exist
        files_to_upload = [f for f in csv_files if f.stem not in existing_tables]
        skipped_files = [f.name for f in csv_files if f.stem in existing_tables]
        
        if skipped_files:
            logger.info(f"Skipping {len(skipped_files)} files whose tables already exist")
            self.state.uploaded_files.extend(skipped_files)

        if not files_to_upload:
            return StepResult(
                status=StepStatus.SKIPPED,
                message=f"All files already uploaded (tables exist)",
                duration_seconds=time.time() - start,
                details={"skipped": skipped_files},
            )

        # Use parallel upload for better performance
        def upload_progress(file_name: str, status: str):
            if status == "success":
        ...
        try:
            results = self.onelake_client.upload_files(
            ...
            uploaded = [r["name"] for r in results.get("success", [])]
            failed = [r["name"] for r in results.get("failed", [])]
            
            self.state.uploaded_files.extend(uploaded)
            ...
            for csv_file in files_to_upload:
                ...
                try:
                    ...
                    logger.error(f"Failed to upload {csv_file.name}: {upload_error}")
        ...
        if failed:
            return StepResult(
                status=StepStatus.FAILED,
                message=f"Uploaded {len(uploaded)} files, {len(failed)} failed, {len(skipped_files)} skipped",
                duration_seconds=time.time() - start,
                details={"uploaded": uploaded, "failed": failed, "skipped": skipped_files},
            )

        return StepResult(
            status=StepStatus.COMPLETED,
            message=f"Uploaded {len(uploaded)} files, {len(skipped_files)} skipped (already exist)",
            duration_seconds=time.time() - start,
            details={"uploaded": uploaded, "skipped": skipped_files},
        )
    ...
    def _step_load_tables(self) -> StepResult:
        ...
        # Configure parallel loading with retry
        parallel_config = ParallelLoadConfig(
            max_workers=self.config.options.max_parallel_workers,
            retry_config=RetryConfig(max_retries=self.config.options.max_retries),
            timeout_per_table=self.config.options.timeout_seconds,
        )
        
        logger.info(f"Loading {len(files_to_load)} tables (max_workers={parallel_config.max_workers}, max_retries={parallel_config.retry_config.max_retries})")
        ...
        for i, table_config in enumerate(table_configs):
            ...
            try:
                ...
                if self.state.lakehouse_id:
                    ...
                    # OneLake path format: https://{onelake_host}/{workspace_id}/{item_id}/Files/eventhouse/{file}.csv
                    # The ;impersonate suffix tells KQL to use the caller's identity to access OneLake
                    from demo_automation.platform.onelake_client import get_onelake_url
                    onelake_path = (
                        f"{get_onelake_url()}/"
    ...
    def _step_configure_bindings(self) -> StepResult:
        ...
        # Format: entity_id -> {"NonTimeSeries": binding_id, "TimeSeries": binding_id}
        existing_entity_binding_ids_by_type = {}  # entity_id -> {binding_type: binding_id}
        ...
        for part in parts:
            ...
            # Extract existing binding IDs with their types (to reuse and avoid duplicates)
            # Path format: EntityTypes/{entity_id}/DataBindings/{binding_id}.json
                ...
                if len(parts_split) >= 4:
                    ...
                    # Decode the binding to get its type (NonTimeSeries or TimeSeries)
                    binding_type = "NonTimeSeries"  # Default
                    try:
                        binding_config = payload.get("dataBindingConfiguration", {})
                        binding_type = binding_config.get("dataBindingType", "NonTimeSeries")
                    except Exception:
                        pass
                    
                    # Store binding ID keyed by entity + type
                    if entity_id not in existing_entity_binding_ids_by_type:
                        existing_entity_binding_ids_by_type[entity_id] = {}
                    existing_entity_binding_ids_by_type[entity_id][binding_type] = binding_id
                    logger.info(f"Found existing {binding_type} binding for entity {entity_id}: {binding_id}")
        ...
        total_bindings = sum(len(b) for b in existing_entity_binding_ids_by_type.values())
        logger.info(f"Found {total_bindings} existing entity bindings (across {len(existing_entity_binding_ids_by_type)} entities), {len(existing_rel_contextualization_ids)} existing contextualizations")
        ...
        if self.state.lakehouse_id and yaml_config.lakehouse_entities:
            for entity_binding in yaml_config.lakehouse_entities:
                ...
                # Reuse existing NonTimeSeries binding ID if available (prevents duplicate bindings)
                entity_bindings = existing_entity_binding_ids_by_type.get(entity_id, {})
                existing_binding_id = entity_bindings.get("NonTimeSeries")
                if existing_binding_id:
                    logger.info(f"Reusing existing NonTimeSeries binding ID for {entity_name} ({entity_id}): {existing_binding_id}")
                else:
                    logger.debug(f"No existing NonTimeSeries binding ID for {entity_name} ({entity_id}), will generate new UUID")
        ...
        if self.state.eventhouse_id and self.state.kql_database_id and yaml_config.eventhouse_entities:
            ...
            for entity_binding in yaml_config.eventhouse_entities:
                ...
                # Reuse existing TimeSeries binding ID if available
                # This allows proper update of existing eventhouse bindings
                entity_bindings = existing_entity_binding_ids_by_type.get(entity_id, {})
                eventhouse_binding_id = entity_bindings.get("TimeSeries")
                
                if eventhouse_binding_id:
                    logger.info(f"Reusing existing TimeSeries binding ID for {entity_name}: {eventhouse_binding_id}")
                else:
                    logger.debug(f"Generating new binding ID for {entity_name} TimeSeries binding")
