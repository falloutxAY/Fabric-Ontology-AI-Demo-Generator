...
Examples:
  ...
  python -m demo_automation cleanup ./Demo --force-by-name   # Cleanup without state file
    ...
    setup_parser.add_argument(
        ...
        type=int,
        default=4,
        help="Max parallel workers for table loading (default: 4)",
    )
    setup_parser.add_argument(
        "--max-retries",
        type=int,
        default=3,
        help="Max retry attempts for failed table loads (default: 3)",
    )
...
def _config_init(args: argparse.Namespace) -> int:
    ...
    console.print("[bold]Environment Selection:[/bold]")
    console.print("  [cyan]1[/cyan] - production (app.fabric.microsoft.com - default)")
    console.print("  [cyan]2[/cyan] - custom (specify your own API URLs)")
    env_choice = Prompt.ask(
        "Select environment",
        choices=["1", "2"],
        default="1",
    )
    env_map = {"1": "production", "2": "custom"}
    environment = env_map[env_choice]
    
    # Get custom URLs if needed
    custom_api_base = None
    custom_api_scope = None
    if environment == "custom":
        console.print("\n[bold]Custom Environment URLs:[/bold]")
        custom_api_base = Prompt.ask(
            "Enter API base URL (e.g., https://api.fabric.microsoft.com)",
        )
        custom_api_scope = Prompt.ask(
            "Enter API scope (e.g., https://api.fabric.microsoft.com/.default)",
            default=f"{custom_api_base}/.default" if custom_api_base else "",
        )
    ...
    console.print(f"  API Base: {config.get_api_base()}")
    console.print(f"  OneLake: {config.get_onelake_url()}")
...
def _config_show() -> int:
    ...
    # Don't apply environment overrides - just load raw config
    ...
    # Determine source for each value (check original env vars, not ones we set)
    import os
    
    # These are user-set env vars that override config
    ...
    # For API URLs, they come from environment config section
    api_source = f"config ({config.environment})"
    ...
    table.add_section()
    table.add_row("[bold]Environment[/bold]", "", "")
    table.add_row("  Name", config.environment, "config")
    table.add_row("  API Base", config.get_api_base(), api_source)
    table.add_row("  OneLake", config.get_onelake_url(), api_source)
    table.add_section()
...
def run_setup(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        config.options.max_parallel_workers = getattr(args, 'parallel_workers', 4)
        config.options.max_retries = getattr(args, 'max_retries', 3)
...
def main() -> int:
    ...
    # Supports: .env in current directory, or Demo-automation/.env
    from dotenv import load_dotenv
    from pathlib import Path
    
    # Try loading from current directory first
    if not load_dotenv():
        # Try loading from Demo-automation directory (for when running from repo root)
        demo_automation_env = Path(__file__).parent.parent.parent.parent / ".env"
        if demo_automation_env.exists():
            load_dotenv(demo_automation_env)
    
    # Load global config and apply environment settings
    # This ensures API base URLs are set before any SDK/client initialization
    config = GlobalConfig.load()
