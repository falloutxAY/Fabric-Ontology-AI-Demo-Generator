...
def get_onelake_url() -> str:
    """Get OneLake URL from environment (set by GlobalConfig.apply_environment_variables)."""
    return os.environ.get('ONELAKE_URL', 'https://onelake.dfs.fabric.microsoft.com')
...
class OneLakeConfig:
    ...
    account_url: str = None  # Will use get_onelake_url() if not provided
    
    def __post_init__(self):
        if self.account_url is None:
            self.account_url = get_onelake_url()
...
class OneLakeDataClient:
    ...
    def __init__(
        ...
        self.account_url = account_url or get_onelake_url()
