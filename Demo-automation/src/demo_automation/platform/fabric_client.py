...
# Fabric API endpoints - configurable via environment variables
def get_fabric_base_url() -> str:
    """Get Fabric API base URL (reads env at call time for dynamic config)."""
    return os.environ.get("FABRIC_API_BASE", "https://api.fabric.microsoft.com") + "/v1"

def get_fabric_scope() -> str:
    """Get Fabric API scope (reads env at call time for dynamic config)."""
    return os.environ.get("FABRIC_API_SCOPE", "https://api.fabric.microsoft.com/.default")

def rewrite_url_for_environment(url: str) -> str:
    """
    Rewrite URLs returned by API (e.g., LRO location headers) to match configured environment.
    
    If a custom API base is configured, rewrites production hostnames in returned URLs
    to use the custom hostname.
    """
    configured_base = os.environ.get("FABRIC_API_BASE", "https://api.fabric.microsoft.com")
    
    # If we're using a custom environment, rewrite production URLs to match
    if configured_base != "https://api.fabric.microsoft.com":
        if "api.fabric.microsoft.com" in url:
            new_url = url.replace("https://api.fabric.microsoft.com", configured_base)
            logger.debug(f"Rewrote URL: {url} -> {new_url}")
            return new_url
    
    return url

# For backward compatibility - these are deprecated, use functions instead
FABRIC_BASE_URL = get_fabric_base_url()
FABRIC_SCOPE = get_fabric_scope()
...
class FabricClient:
    ...
    def _get_token(self) -> str:
        ...
        try:
            token = self._credential.get_token(get_fabric_scope())
    ...
    def _make_request(
        ...
        # Rewrite URL for environment (handles LRO URLs that may use wrong host)
        url = rewrite_url_for_environment(url)
    ...
    def _build_url(self, path: str) -> str:
        ...
        if path.startswith("http"):
            return path
        return f"{get_fabric_base_url()}/workspaces/{self.workspace_id}/{path.lstrip('/')}"
    ...
    def create_item(
        ...
        # Handle ItemDisplayNameNotAvailableYet - auto-retry
        if response.status_code == 400 and _retry_count < max_retries:
            try:
                error_data = response.json()
                error_code = error_data.get("errorCode", "")
                if error_code == "ItemDisplayNameNotAvailableYet":
                    logger.warning(
                        f"Name '{display_name}' temporarily reserved. "
                        f"Waiting {retry_wait}s before retry ({_retry_count + 1}/{max_retries})..."
                    )
                    if progress_callback:
                        progress_callback(
                            f"Name temporarily reserved, waiting {retry_wait}s...", 0.0
                        )
                    time.sleep(retry_wait)
                    # Retry the creation
                    return self.create_item(
                        item_type, display_name, description, definition,
                        timeout_seconds, progress_callback, _retry_count + 1
                    )
